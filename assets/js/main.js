...
$("body").on("click", ".product .call-modal-add-to-cart", function(){
    ...
    window.setTimeout(function(){$(".modal-add-to-cart").removeClass("active");}, 5000);
});

$("body").on("click", ".product-popup .call-modal-add-to-cart", function(){
    var name = $(this).parents(".product-popup").find(".product-popup__title").text();
    var quantity = $(this).parents(".product-popup").find(".stepper__input").val();
    $(".modal-add-to-cart p span").text(quantity + " " + name);
    $(".modal-add-to-cart").addClass("active");
    window.setTimeout(function(){$(".modal-add-to-cart").removeClass("active");}, 5000);
});
...
$(".counter .minus").on("click", function(){
    ...
    if ("1" != e){
        $(this).next("input").val(e - 1);
    }
});

$(".counter .plus").on("click", function(){
    var e = parseFloat($(this).prev("input").val());
    $(this).prev("input").val(e + 1);
});
...
if($(window).width() < 1025){
    ...
    $(".horizontal-blocks .image-heading-block").wrapAll( "<div class='swiper-wrapper' />");
    ...
    $(".site-section--horizontal-blocks .pagination-counter-total").text(swiper2.slides.length);

    swiper2.on('transitionEnd', function() {
      $(".site-section--horizontal-blocks .pagination-counter-current").text(swiper2.realIndex + 1);
    });
} else {
